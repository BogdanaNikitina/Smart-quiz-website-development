...
function renderStep() {
    ...
    if (step.type === "form") {
        ['form_name', 'form_phone', 'form_comment'].forEach(id => {
            let el = document.getElementById(id);
            if (el) el.addEventListener('input', () => syncCurrentStepToAnswers());
        });
        let emailEl = document.getElementById('form_email');
        if (emailEl) emailEl.addEventListener('input', () => syncCurrentStepToAnswers());
        let consentEl = document.getElementById('form_consent');
        if (consentEl) consentEl.addEventListener('change', () => syncCurrentStepToAnswers());
    }
...
async function submitFinalForm() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // Показываем экран с данными из предыдущего опроса
            showAlreadySubmittedScreen(result.existing_data);
        } else {
...
function showAlreadySubmittedScreen(existingData) {
    if (!stepsContainer) return;
    const zonesText = existingData.zones && existingData.zones.length ? existingData.zones.join(', ') : '—';
    const formattedDate = existingData.created_at ? new Date(existingData.created_at).toLocaleString('ru-RU') : 'ранее';
    
    stepsContainer.innerHTML = `
        <div class="success-message">
            <h2>С возвращением!</h2>
            <p>Вы уже проходили опрос ранее ${formattedDate}.<br>Ниже приведены данные из вашей предыдущей отправки.</p>
            <div class="data-card">
                <strong>Ваши данные из предыдущего опроса:</strong><br><br>
                <p><strong>Тип помещения:</strong> ${escapeHtml(existingData.room_type || '—')}</p>
                <p><strong>Зоны:</strong> ${escapeHtml(zonesText)}</p>
                <p><strong>Площадь:</strong> ${escapeHtml(existingData.area)} м²</p>
                <p><strong>Стиль:</strong> ${escapeHtml(existingData.style || '—')}</p>
                <p><strong>Бюджет:</strong> ${escapeHtml(existingData.budget || '—')}</p>
                <hr>
                <p><strong>Имя:</strong> ${escapeHtml(existingData.name || '—')}</p>
                <p><strong>Телефон:</strong> ${escapeHtml(existingData.phone)}</p>
                <p><strong>E-mail:</strong> ${escapeHtml(existingData.email || '—')}</p>
                <p><strong>Комментарий:</strong> ${escapeHtml(existingData.comment || '—')}</p>
            </div>
            <p style="text-align: center; margin-top: 20px;">Мы свяжемся с вами в ближайшее время по указанному номеру телефона.</p>
            <button class="reset-btn" id="goHomeBtn">На главную →</button>
        </div>
    `;
...
function showPage(pageId) {
    if (homePage) homePage.classList.remove('active-section');
    if (aboutPage) aboutPage.classList.remove('active-section');
    if (quizPage) quizPage.classList.remove('active-section');
    if (pageId === 'home' && homePage) homePage.classList.add('active-section');
    else if (pageId === 'about' && aboutPage) aboutPage.classList.add('active-section');
    else if (pageId === 'quiz' && quizPage) { 
        quizPage.classList.add('active-section'); 
        setTimeout(initQuiz, 50); 
    }
    navItems.forEach(i => { 
        if (i.getAttribute('data-page') === pageId) i.classList.add('active'); 
...
if (mainStartBtn) mainStartBtn.addEventListener('click', () => showPage('quiz'));
if (startQuizBtn2) startQuizBtn2.addEventListener('click', () => showPage('quiz'));
